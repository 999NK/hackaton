...
export function ScoreCard({ wcag }: { wcag?: WcagReport }) {
  ...
  return (
    <section className="surface-card overflow-hidden p-6 sm:p-8">
      <div className="mb-6 flex items-center gap-2"><ShieldCheck size={19} className="text-blue-600" /><h2 className="text-lg font-semibold text-slate-950">Score de acessibilidade</h2></div>
      <div className="flex flex-col justify-between gap-7 lg:flex-row lg:items-center">
        <div className="flex items-center gap-5"><div className={`flex items-end rounded-[24px] px-6 py-5 ${scoreStyle(wcag.score)}`}><strong className="text-6xl font-semibold tracking-[-0.06em]">{wcag.score}</strong><span className="mb-1.5 ml-1 text-lg font-semibold opacity-60">/100</span></div><div><p className="text-sm text-slate-400">Conformidade alcançada</p><Badge className="mt-2 border-blue-200 bg-blue-50 px-3 py-1 text-blue-700 hover:bg-blue-50">Nível {wcag.level}</Badge><p className="mt-3 text-xs text-slate-400">Regras {wcag.rulesVersion}</p></div></div>
