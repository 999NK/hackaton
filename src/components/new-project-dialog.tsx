...
export function NewProjectDialog({ onCreate }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!loading) { setOpen(next); if (!next) setName('') } }}>
      <DialogTrigger asChild><Button className="h-11 gap-2 rounded-full bg-violet-600 px-5 font-semibold text-white shadow-lg shadow-violet-200 hover:bg-violet-700"><Plus size={17} /> Novo projeto</Button></DialogTrigger>
      <DialogContent className="overflow-hidden rounded-[26px] border-slate-200 bg-white p-0 text-slate-950 shadow-2xl sm:max-w-[500px]">
        <div className="border-b border-violet-100 bg-gradient-to-b from-violet-50/80 to-white px-7 pb-6 pt-8 sm:px-8">
          <div className="mb-5 grid h-12 w-12 place-items-center rounded-2xl bg-violet-600 text-white shadow-lg shadow-violet-200"><FolderPlus size={23} /></div>
        ...
        <form onSubmit={handleSubmit} className="px-7 pb-7 pt-6 sm:px-8 sm:pb-8">
          <div className="space-y-2"><Label htmlFor="project-name" className="text-sm font-semibold text-slate-700">Nome do projeto</Label><Input ref={inputRef} id="project-name" name="name" autoComplete="off" maxLength={80} required value={name} onChange={(event) => setName(event.target.value)} className="h-12 rounded-xl border-slate-200 bg-white px-4 text-slate-900 shadow-sm placeholder:text-slate-400 focus-visible:border-violet-400 focus-visible:ring-4 focus-visible:ring-violet-50" placeholder="Ex.: Portal do cliente" /><div className="flex items-center justify-between text-xs"><span className={name.length > 0 && cleanName.length < 2 ? 'text-red-500' : 'text-slate-400'}>{name.length > 0 && cleanName.length < 2 ? 'Digite pelo menos 2 caracteres' : 'Você poderá configurar a URL depois.'}</span><span className="text-slate-300">{name.length}/80</span></div></div>
          <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="ghost" disabled={loading} onClick={() => setOpen(false)} className="h-11 rounded-full px-5 text-slate-500 hover:bg-slate-100 hover:text-slate-900">Cancelar</Button><Button type="submit" disabled={loading || cleanName.length < 2} className="h-11 gap-2 rounded-full bg-violet-600 px-5 font-semibold text-white hover:bg-violet-700 disabled:bg-slate-200 disabled:text-slate-400">{loading ? <><Loader2 size={17} className="animate-spin" /> Criando...</> : <>Criar projeto <ArrowRight size={17} /></>}</Button></div>
        </form>
      </DialogContent>
    </Dialog>
  )
