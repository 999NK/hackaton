...
export default function Dashboard() {
  ...
  return (
    <div className="page-shell py-8 sm:py-10">
      ...
      <div className="relative mb-7 max-w-md"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar projetos..." className="h-12 rounded-full border-violet-100 bg-white pl-12 text-slate-800 shadow-sm placeholder:text-slate-400 focus-visible:border-violet-400 focus-visible:ring-violet-100" /></div>
      {loading ? <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{[1, 2, 3].map((item) => <Skeleton key={item} className="h-[280px] rounded-3xl bg-violet-100" />)}</div> : filtered.length === 0 ? <div className="surface-card border-dashed py-20 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-50 text-violet-600"><FolderPlus size={28} /></div><h3 className="text-xl font-semibold text-slate-900">{search ? 'Nenhum resultado' : 'Seu primeiro projeto começa aqui'}</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">{search ? 'Tente buscar por outro termo.' : 'Crie um projeto para auditar WCAG, mapear a navegação e acompanhar a evolução do score.'}</p></div> : <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{filtered.map((project) => <ProjectCard key={project.id} project={project} latestScan={scansMap.get(project.id)} entityCount={entityCounts.get(project.id) || 0} onDelete={handleDelete} />)}</div>}
    </div>
  )
}
